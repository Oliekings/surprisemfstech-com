...
@section('page_subtitle', 'Update builder profile, role, and social media links')

@php
    $socials = [];
    if (is_array($member->social_links)) {
        foreach ($member->social_links as $item) {
            if (is_array($item) && !empty($item['platform']) && !empty($item['url'])) {
                $key = strtolower($item['platform']);
                if ($key === 'x') $key = 'twitter';
                $socials[$key] = $item['url'];
            }
        }
    }
@endphp
...
<div class="max-w-2xl" x-data="{
    avatarPreview: '{{ $member->avatar_path }}',
    previewAvatar(event) {
    ...
    <form method="POST" action="{{ route('admin.team.update', $member) }}" enctype="multipart/form-data" class="p-6 lg:p-8 rounded-3xl bg-zinc-950/80 border border-zinc-800/80 space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="flex-1 space-y-1.5">
                    ...
                    <input 
                        type="text" 
                        name="avatar_url" 
                        value="{{ old('avatar_url', $member->avatar_path) }}"
        ...
        <div class="space-y-4 pt-4 border-t border-zinc-800">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1">
                    ...
                    <input 
                        type="url" 
                        name="instagram_url" 
                        value="{{ old('instagram_url', $socials['instagram'] ?? '') }}" 
                ...
                <div class="space-y-1">
                    ...
                    <input 
                        type="url" 
                        name="linkedin_url" 
                        value="{{ old('linkedin_url', $socials['linkedin'] ?? '') }}" 
                ...
                <div class="space-y-1">
                    ...
                    <input 
                        type="url" 
                        name="twitter_url" 
                        value="{{ old('twitter_url', $socials['twitter'] ?? '') }}" 
                ...
                <div class="space-y-1">
                    ...
                    <input 
                        type="url" 
                        name="github_url" 
                        value="{{ old('github_url', $socials['github'] ?? '') }}" 
            ...
            <div class="space-y-1">
                ...
                <input 
                    type="url" 
                    name="website_url" 
                    value="{{ old('website_url', $socials['website'] ?? '') }}" 
